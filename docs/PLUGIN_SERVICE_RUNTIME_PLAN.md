...
基线：`89294fdef0b84fc08e720017bb4da4a83b6a97c8`；2026-09-20。本文区分已实现的完整所有权、有限服务运行租约、累计任务/字节预算、原声明内的显式续租，以及仍待实现的工作台调度。主应用常驻节点尚未完成。
...
管理队列预留与工作台调度仍待实现；没有自动续租、无限服务时长或超过原声明的补充额度路径。
