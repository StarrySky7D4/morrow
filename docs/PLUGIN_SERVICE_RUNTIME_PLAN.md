...
基线：`be087eee7f593a0705dbd53f8fecced3a10b12d6`；2026-09-20。本文区分已实现的完整所有权、有限服务运行租约原型，以及仍待实现的续租、完整预算和工作台调度。主应用常驻节点尚未完成。
...
可信宿主必须显式调用 `Manager::bind_service_run`，核对实际Manager、原Host/instance、当前修订、包摘要和已批准能力。声明本身不授权；读取持久发布配置也不自动签发运行租约。签发在原IoContext锁内一次完成，无效请求不消耗首次签发机会。相同实例不能通过丢弃句柄、再次绑定、旧bind_io入口或到期后重绑续期。新实例需要重新显式批准，不恢复旧租约。
...
下列完整预算和续租合同仍待实现；当前仅有限运行期限、原并发容量和累计字节账本落地。
...
固定且显式批准的有限运行租约已实现，下一步增加完整运行预算后再实现显式续租。续租必须绑定同一run、原账本和期望修订，核对当前Manager/配置/认证批准；以新的绝对累计上限和期限替换批准，不把“增加额度”解释为抹去已经使用的额度。自动轮换绑定、时钟回退、过期后复活及跨run转移批准均拒绝。累计作业计数及续租额度尚未实现，后续数值上限需经原型测量确定并版本化；不能把Usage.jobs并发数误报为累计作业总额，也不能把当前一小时有限原型当作整套预算完成。
