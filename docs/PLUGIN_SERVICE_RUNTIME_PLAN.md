...
历史阶段记录（后续实现以本文顶部最新报告为准）：`41e6431`；2026-09-20，版本 `0.1.9-test.52+56`。已实现完整WorkbenchState、原执行者命令预留、内部续租、本地/worker共用业务派发，以及原生应用的持久配置服务准入和监督回收。当前应用服务准入限定明确批准的单个loopback HTTP有限运行；私有异步命令协议、有界身份表及Dart业务自动路由已接入；Flutter有限服务运行面板及启动诊断已接入，真实端口占用/到期/撤销发布与认证/准入拒绝8项通过；实际窗口与其余故障、应用服务TLS和出站资源适配仍待完成。现有短IO自动drain的行为不变。
...
精确撤销的退出门槛：端点修改只使依赖该端点的活动引用失效；凭据轮换使所有依赖该凭据的端点失效；配置/发布/认证变更使关联服务失效；原Store销毁和显式全局撤销仍使全部租约失效。保持原Store协调锁与CAS，撤销先于修改，失败/回滚不能复活旧权限；未知结果不重发。必须补关联与无关资源并存、跨Store/重连、回滚/提交不确定及真实等待回收测试后才切换主应用策略。
