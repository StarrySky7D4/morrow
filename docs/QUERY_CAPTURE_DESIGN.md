...
test.44 已在同一数据库实现有序类型化分片与摘要链，最终目录原子发布。test.45新增固定已验证游标与全库32档／8 GiB暂存硬准入。详见 [分片契约](PLUGIN_READ_ARCHIVE.md)。默认查询入口尚未使用该持久化适配器。
...
3. 有界分片（test.44底座及故障资格已验，test.45已有全库暂存准入，已发布历史配额仍待实现）：跨现有单份容量完成目录；故障注入覆盖准备、最终提交前后、缺片／重复和配额耗尽，不发布部分成功。
4. 默认查询接入：使用实际 Rust guest 记录全部三阶段调用，重建过滤、稳定排序及归并；空库同样有真实任务。
5. 恢复与独立核验（test.44实际guest资格已验，test.45改为固定游标；默认入口及独立查询CLI仍待实现）：删除原 guest 文件后用原包重放，核对来源材料与最终目录；稳定重试不读最新数据改结果。
