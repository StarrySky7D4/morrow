...
当前阶段为 test.44；读取日志基础见 [PLUGIN_READ_JOURNAL.md](PLUGIN_READ_JOURNAL.md)。
...
## 续接分片与原子最终目录（test.44底座与资格适配器已实现）

全库候选和所有调用可能超过一份 Evidence；不能把查询缩成前 N 张卡或省略归并观察。
test.44 已在同一数据库实现有序类型化分片与摘要链，最终目录原子发布。详见 [分片契约](PLUGIN_READ_ARCHIVE.md)。默认查询入口尚未使用该持久化适配器。
...
3. 有界分片（test.44底座及故障资格已验，全局配额仍待实现）：跨现有单份容量完成目录；故障注入覆盖准备、最终提交前后、缺片／重复和配额耗尽，不发布部分成功。
4. 默认查询接入：使用实际 Rust guest 记录全部三阶段调用，重建过滤、稳定排序及归并；空库同样有真实任务。
5. 恢复与独立核验（test.44实际guest资格已验；默认入口及独立查询CLI仍待实现）：删除原 guest 文件后用原包重放，核对来源材料与最终目录；稳定重试不读最新数据改结果。
