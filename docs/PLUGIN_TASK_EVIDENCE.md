# 纯转换历史证据与隔离重放：test.34

本阶段建立实际捕获、可保存原件和独立执行验证。它不代表内容已经提交、证据已经签名或完整多包流程可以重放。默认工作台尚未自动保存此证据；现有内容存储与审计路径保持其原职责。
...
Evidence 保留原始 PB、容器和摘要，未知可选 PB 字段不因解析重编码丢失。decode 必须传入外部固定的原始 PB SHA-256；它与压缩文件 SHA-256 不同。容器不自行建立信任，摘要也不证明是谁执行了任务。core 的 encode 是格式工具，调用者可以构造观察数据；只有 runtime 的捕获入口自行执行并形成不可直接构造的 CapturedTransform，但导出文件仍需要后续审计关联来证明来源。
...
CapturedTransform 提供实际 TaskReport 与 Evidence。它不提供核心提交证明、对象授权或可复活句柄，也没有写入生产数据库。读取历史证据不依赖原注册表、模块文件、运行实例继续存在。
...
现有工作台 `run/transform` 只返回解码数据，创建／编辑随后才构造核心内容命令；设置保存还存在多次分页任务对应一次提交。下一步应保留有序证据集合到实际提交边界，扩展现有 Store::apply 事务共同写入证据、操作引用、内容、回执及包含证据摘要的原始 Commit，不能在提交后另开文件补写。

证据可按摘要在同一库去重，幂等操作不能被重试产生的新证据替换。最终授权检查保持；审计封存实际签署引用证据的原始 Commit 后才形成签名关联。完整性检查、快照和导出需携带并核对整个证据闭包。纯任务复现之后，还需记录版本化宿主投影、前置内容与最终命令，才能验证工作台完整内容重建。

多层 A/B 调用证据、宿主响应录制、签名关联、持久化配额／崩溃恢复、保留与清理状态、完整隔离重放及六平台验收继续推进。详见 [本轮验证](../reports/test.34-transform-replay.md)。
