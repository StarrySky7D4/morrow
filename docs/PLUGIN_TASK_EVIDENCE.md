...
test.38 的 CLI 按证据版本分派：schema 1 使用 `replay`，schema 2 使用 `replay_batch`。它只读打开有界证据文件，始终要求外部原始 PB 摘要 pin；逐页策略使用默认 `Limits`，批次总燃料固定为 1,000,000,000（1B）。通用 Rust API 可由可信调用方指定最高 10,000,000,000（10B）的总预算，CLI 不因此自动放宽。退出 0 表示精确观察匹配，2 表示不匹配，1 表示格式、完整性、策略或后端拒绝。标准输出报告观察数，不打印用户输入／输出正文。本项新增6项实际CLI进程测试与严格Clippy均已通过；原合成宿主库及目录在执行CLI前删除，测试覆盖两版本匹配、错pin／损坏、超总策略及不匹配退出。
...
test.35 时，默认工作台 `run/transform` 仍主要返回解码数据，创建／编辑随后构造核心内容命令；设置保存还存在多次分页任务对应一次提交。下一步需要保存从捕获到提交的稳定意图与有序原件，处理重试，再接入默认 UI 自动捕获。还需记录版本化宿主投影、前置内容与最终命令，才能验证完整内容重建。
