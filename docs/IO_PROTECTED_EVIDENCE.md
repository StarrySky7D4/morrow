...
状态：core 存储层已有 Store v17 限定实现；修正与验证范围见 [整合修正报告](../reports/io-safety-refactor-2026-09-19.md)。本层只保存**历史材料**：固定原件既不是发送许可、不是凭据存储，也不证明远端成功或用户收到结果。Broker 的当前实例绑定、唯一活跃执行与恢复核对属于 IO-C2，不在此文件范围内。
...
`Material::decode` 在分配前检查字段号、重复字段、线类型与长度，要求 Protobuf 重新编码与解压后的原字节一致，接受后保留收到的完整 LZ4 容器，不重新压缩，并校验 `payload_bytes == payload.len()`、`payload_sha256 == SHA-256(payload)`。未知字段或语义必须走显式新版本。记录自身的 SHA-256（`digest`）用作存储键，但摘要不证明材料存在或远端成功。
...
2. 原件的 `request_sha256` 必须等于该命令的 `request_sha256`；解码后的字段与列值逐一相符。请求载荷长度与摘要须与命令完全一致，响应须在命令上限内且已有发送边界。
3. 同一 `(operation_id, kind)` 不能同时存在原件与预留（消费式转换，不是复制）。
4. 使用真实末尾 phase 判断终态，不以固定 revision 数字猜测。预留不得与终态（`Observed`／`CancelledBeforeDispatch`）历史共存；`OutcomeUnknown` 期间响应预留可合法保留，直到响应原件被接纳或历史终结。
...
`reserve_io_materials` 在跨越发送边界前为同一命令建立完整材料预留：请求按 `command.request_bytes`、响应按 `command.response_limit` 的容器上界计入。预留是 Store 内的逻辑配额，通过 `byte_room` 从所有 outbox 写入者共享的字节额度中扣除；材料预留不产生事件，因此不占事件计数额度，其他写入无法挤占；它不是物理磁盘空间保证，SQLite 真实满盘仍以 `StorageFull` 报告。相同 operationId 幂等不重复扣减，不同命令或不同主体拒绝。
...
- 存在但不满足绑定：`Integrity`。读取、幂等重试与完整性核验共用有界载入；SQL 先限制材料和命令历史 BLOB 长度，再分配解码。幂等重试也验证已存原件，不能把损坏的旧行当成功。
