...
这是已批准依赖锁之上的单层同步转换接口。test.31另外提供显式 [多层图入口](PLUGIN_DEPENDENCY_GRAPH.md)，本页单层限制仍适用于原run入口。默认工作台界面尚未接入依赖配置或这条执行路径。依赖声明和同快照批准规则沿用 [依赖锁](PLUGIN_DEPENDENCY_LOCKS.md)，完整插件系统与0.2.0门槛仍未完成。
