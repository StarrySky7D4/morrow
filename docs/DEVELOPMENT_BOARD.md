...
当前依次推进：真实窗口与内容/UI/capture共存 → 业务提交后丢回执、慢回调停止、封存修复 → 长IO可暂停与超限帧分段 → 应用服务TLS/出站资源。启动回执损坏/EOF已在真实宿主验证，完整故障恢复仍未完成。文件系统可独立推进；跨重启Unknown、因果证据、三语言IO SDK和全平台资格未完成。应用服务当前只准入已批准的单个回环HTTP有限运行。
