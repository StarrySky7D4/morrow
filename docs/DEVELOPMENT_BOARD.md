...
| 3 / IO-B2 | 已验调度子集；接线下一项 / P0 | runtime 作业调度＋独立契约路由 | 有界 submit/poll/read/cancel、Ready 最终交付撤权、声明预算、温和排空已验；下一步把独立可信 Router 接入 Manager/Pool 的 IoBinding＋Broker，贯通批准、同一配额与真正后端 |
