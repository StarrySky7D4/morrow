...
| 5 / IO-E2 | 已验类别、凭据、端点与Rust任务状态子集，整体进行中 / P1 | workbench_host＋Flutter 管理界面 | [管理接口](PLUGIN_IO_MANAGEMENT.md)已接私有协议与真实Registry：声明／批准分离、明确保存／撤销、修订校验与重启恢复；原Store有界分页、Windows凭据与[具体端点批准](PLUGIN_ENDPOINT_MANAGEMENT.md)录入／替换／停用已验。[Rust应用任务状态](PLUGIN_APP_IO_TASKS.md)已接原Storage所有权、Busy与恢复；仍需CLI／Flutter任务消息与关闭待退出，再验用户路径和API服务，用户资料无隐式迁移 |
