...
已添加 sdk/c、sdk/cpp、sdk/rust 的实验传输层。C 提供静态库；C++17 提供 C ABI 上的便捷包装；Rust 提供无第三方依赖的独立 crate。它们复制请求、提交前准备有界输出、区分传输失败与业务响应且不自动重试。没有导出创建宿主、授予权限、打开数据库或自选插件身份的接口。

当前仍需调用者使用固定绑定准备和解码 Cap’n Proto；SDK 尚未提供内容／UI 的完整类型化便捷 API、异步任务、mmap 租约、包工具及 Wasm guest 导入。mp_host_v1／HostV1 只是实验适配接口，既不是稳定插件 ABI，也不是跨进程传输格式。原生指针只在测试适配器本地使用。
