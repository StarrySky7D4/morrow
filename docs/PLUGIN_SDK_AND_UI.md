...
当前可直接构造重命名、摘要查询、操作结果查询和附件片段请求，并校验响应契约及关联。SDK 仍缺通用记录／UI 的完整类型化 API、完整附件装配验证、异步任务、mmap 租约、包工具及三语言完整 Wasm guest 适配。已增加 Rust guest 导入和 Windows Wasmi 执行原型，验证权限绑定、限额和提交后故障；证据见 [执行后端说明](../plugin_runtime/README.md)。mp_host_v1／HostV1 只是实验适配接口，既不是稳定插件 ABI，也不是跨进程传输格式。原生指针只在测试适配器本地使用。
