...
2026-09-15 补充：后续任务按 [执行路线修订](ROADMAP_UPDATE_2026-09-15.md) 推进。新增稳定业务 ID、类型化本地化消息及独立版本路由；保留 UI1 与冻结原包。C／C++／Rust 继续为开发入口，受控原生插件是另行验证的后端，Android 随包执行引擎不等同开放原生插件。
...
动态内容命令任务及 C／C++／Rust 输入／完成 API 已建立，见 [任务契约](PLUGIN_TASK_PROTOCOL.md)。纯转换输入与输出已接通；跨包 handler 选择、类型协商、修改提案和实际插件事件桥接仍未完成；基础 UI schema 见下方新增协议，首期语言边界保持不变。

单包纯转换处理器注册已实现，三语言共用打包工具声明类型和上限；宿主执行前匹配、输出后复核。见 [插件包声明](PLUGIN_PACKAGE.md#纯转换处理器声明)。此注册只覆盖纯转换；UI 扩展点注册仍待实现，基础 Flutter 渲染见后述增量。
