...
当前可直接构造重命名、摘要查询、操作结果查询和附件片段请求，并校验响应契约及关联。SDK 仍缺通用记录／UI 的完整类型化 API、完整附件装配验证、异步任务、mmap 租约、完整包管理及三语言生产执行后端适配。已增加 C／C++／Rust guest 导入和 Windows Wasmi 执行原型，三种语言共享编解码及授权链路，验证权限绑定、限额和提交后故障；证据见 [执行后端说明](../plugin_runtime/README.md)。mp_host_v1／HostV1 只是实验适配接口，既不是稳定插件 ABI，也不是跨进程传输格式。原生指针只在测试适配器本地使用。
...
原生后台队列与取消／排空已有可运行增量，见 [任务接口](PLUGIN_TASKS.md)。目前由可信 Rust 宿主使用，仍缺三语言统一任务输入契约和 Flutter UI 事件桥接，不能把后台示例当作 UI 对接完成。

动态内容命令任务及 C／C++／Rust 输入／完成 API 已建立，见 [任务契约](PLUGIN_TASK_PROTOCOL.md)。通用计算／转换任务与声明式 UI schema、渲染器和事件桥接仍未完成，首期语言边界保持不变。
