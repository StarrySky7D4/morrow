...
状态：核心协议层已实现并通过验收；这是 IO-D1 的第一步，**不表示真实出站已经可用**。真实网络路径、origin／方法／凭据授权与 Unknown 核对仍未接入。
...
- 方法只允许非空大写 ASCII token；目标是 origin-form（以 `/` 开头、不含 `://`、不含 `#`、无控制字符与空白）。
- 请求头：名字为 HTTP token，值不得含 CR/LF/NUL；数量 ≤64、名字 ≤128、单值 ≤8 KiB、总量 ≤16 KiB。
- `host`、`connection`、`content-length`、`transfer-encoding`、`upgrade`、`te`、`trailer`、`expect`、`proxy-authorization`、`proxy-connection`、`keep-alive` 由运行时掌控，guest 提交一律拒绝——避免改写被钉定的 origin 或分帧。
...
本步不发起任何网络请求，不创建授权，不保存证据，也不改变冻结 SDK。数据库格式与运行时行为未变。
