...
这是不可变 `.mplugin` 安装目录之上的原生持久状态底座，提供 `morrow_core::plugin_package::registry::Registry`。受管理的原生运行入口和既有实例撤权已由 [Manager](PLUGIN_MANAGER.md) 接入；注册表本身不持有实例。工作台插件管理页面仍独立推进，低层包适配器仍可由可信代码直接调用，因此不能把本模块描述为全局执行隔离或完整插件管理器。
...
批准集合仅为宿主后续授权的上限，不是可转让 grant。可信集成者必须在创建连接、发放具体对象授权时取此集合与其他政策的交集，并在禁用、升级、批准收窄时停止或撤销既有实例。注册表模块本身不持有实例句柄；Manager 独占它并管理上述连接上限及撤权。直接使用低层 Catalog、PreparedPackage 或 HostRuntime 的可信代码不受该入口强制门控。
...
注册表全体状态损坏或选中包缺失时保留证据并失败，不允许自动清空。恢复／修复工具、历史选择和撤销记录、签名及发布者信任、升级中的运行任务收束、用户可见安装管理均为后续工作。
...
selection.morrow新增规范依赖锁，和包选择同次PB＋LZ4原子持久化、共用revision及独占锁。旧无依赖快照兼容；有依赖语义的未知旧读取器拒绝改写。批准／移除采用CAS，必需缺失与启动环拒绝连接；升级／移除清除相关双向锁，停用保留配置。Registry不撤销运行实例，Manager在持久变更之前完成旧图撤权。见 [设计与边界](PLUGIN_DEPENDENCY_LOCKS.md)。
