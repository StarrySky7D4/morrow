...
`Workbench` 的 Rust 应用入口使用显式 `StorageSlot` 承接完整原 Storage。该入口负责一次任务的准入、交付、停止、实际线程回收与恢复；不创建第二份数据库。它属于 IO-E2 的应用接线，CLI／Flutter 的端点批准页面和任务启动消息仍待接入，不能视为已经交付网络任务界面。
...
沿用现有错误字段与 `ui_code`：110＝Busy，111＝需要恢复，112＝原存储不可用，113＝任务身份失效，114＝上一任务尚未确认。未修改消息布局、schema 摘要或冻结 SDK；旧 Dart 错误处理可接收普通错误，查询不能将 Busy 误判为终止。
...
在原 Store 上接入具体端点批准管理，再将经过批准的 HTTP 任务连接到私有 start／poll／read／cancel 消息与 Flutter 界面。准备回调不是插件可提交的任意路由，也不是从 UI 直接构造授权的捷径。随后验证实际应用批准、请求、取消、重启核对及 API 节点管理，继续文件系统、Unknown 核对、完整因果链和三语言 IO SDK；当前契约不承诺 SDK 稳定或全平台运行资格。
