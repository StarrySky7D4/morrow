...
本地访问仍先检查StateSlot；待显式修复时只读入口保留，写入和上传消费被拒绝。worker的原身份/有效性和prepare_io检查在业务派发前执行。当前尚未从主应用向Running服务提交这些命令；测试专用的运行中worker不替代公开准入与生命周期实现。下一步接常驻运行准入、异步命令回执及主应用API节点启动/停止/修复，再实现长IO可暂停与服务期间编辑响应。准备回调不是插件可提交的任意路由，也不是从UI直接构造授权的捷径。文件系统、Unknown核对、完整因果链和三语言IO SDK仍按原门槛推进；当前契约不承诺SDK稳定或全平台运行资格。
