...
状态：运行时作业层已实现并通过验收。它把**完整任务输入**、**每作业契约路由**和**有界队列／调用／字节／期限**组合成一个可 submit／poll／read／cancel 的作业，并保证停止后不把迟到结果当作成功。真实网络或文件后端由 IO-D 提供；本层自身不执行任何 I/O。
...
- `drain(timeout)` 停止接收并收紧所有期限；`stop()` 先撤权再取消；`try_finish()` 只在执行线程结束后归还核心所有权。

## 界限

| 资源 | 上限 | 行为 |
| --- | --- | --- |
| 队列＋运行中 | 64（`capacity` 1..=64） | 超出返回 `Busy`，不阻塞发送 |
| 每次作业调用数 | 1024 | 超出后该调用被拒，作业以 `Limits` 结束 |
| 每作业字节 | 16 MiB | 输入＋每次请求＋每次响应累计；超出即拒绝 |
| 执行器总字节 | 64 MiB | 累计只增不退，释放作业不返还 |
| 期限 | ≤ 3600 秒 | 排队中已过期不启动；运行中超过期限的结果不作成功 |
...
`JobReport` 因此区分三种事实：`execution.outcome` 是运行结论，`cancelled` 表示结果迟到，`unknown` 表示外部效果未知、只能核对。

## 停止与迟到结果

- `stop()` 先撤销实例授权，再取消全部作业；已通过最终授权的调用仍可能提交，但结果不再作为成功交付。
- 作业在运行结束后、交付之前再次检查取消令牌：期限或撤权后到达的结果被替换为 `Cancelled`／`Deadline`，响应被丢弃（`response = None`）。
- 执行器结束而结果未交付时 `read` 返回 `Unavailable`，不把缺失当作成功。
- `read` 的字节上限在消费前检查：超出返回 `ReadBound` 并保留结果，调用方可显式增大上限重试。
