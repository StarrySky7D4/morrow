...
状态：broker 层已实现并通过验收，依赖 [IO-C1 受保护原件](IO_PROTECTED_EVIDENCE.md) 的存储与预留。本层只做一件事：让一个 operationId 在任一时点最多有一次真实外发执行，并让任何中断都只能走核对、不能走重发。真实 HTTP／文件后端、异步作业与重试策略属于 IO-B2／IO-D，不在本文件范围。
...
1. 提供的 subject 与 capability 必须等于命令自身声明；capability 必须在这次绑定的批准集合内，实例、管理器、主机与时钟都必须是当前代次。
...
5. 后端恰好调用一次。
6. 成功后先保留响应原件，再提交 `Observed`，最后释放活跃执行与作业租约。
...
- `maintain(now)` 回收绑定已过期、管理器已释放或实例已停止的活跃执行；`retire(operation)` 可显式回收。
...
真实出站／监听／服务发布声明与 Registry 授权（IO-D1–D3）、异步 submit/poll/read/cancel 与任务输入（IO-B2）、真实后端效果核对来源与远端身份、录制式隔离回放（ROAD-08-IO）、凭据注入、按配额退休、主应用 UI 与三语言 SDK。本层不表示任何插件已经可以访问网络或文件系统。
