...
首轮组合测试有 2 项关闭测试失败：传输错误被清理中的“正在关闭”覆盖。恢复原错误优先级后，关闭专项 3 项及最终组合 69 项全部通过。真实测试最初把单作业 max_calls 设为 64，超出测试包 host_calls=16；将参数调整为合法的 4，没有扩大运行时准入。后续故障测试复核确认：此限制与 IO 声明中的并发 jobs=4 是不同维度。
