...
fn revision_one(c: &Connection, operation: &str) -> Result<Option<Record>> {
    let raw: Option<Vec<u8>> = sql(c.query_row(
        "SELECT o.payload FROM io_intents i JOIN operations o ON o.id=i.event_id WHERE i.operation_id=?1 AND i.revision=1",
        [operation],
        |r| r.get(0),
    ).optional())?;
    raw.map(|bytes| Record::decode(&bytes)).transpose()
}
pub(super) fn verify(c: &Connection) -> Result<()> {
    ...
    let mut statement = sql(c.prepare(
        "SELECT operation_id,subject,kind,request_sha256,payload_sha256,payload_bytes,digest,container FROM io_evidence",
    ))?;
    ...
    while let Some(row) = sql(rows.next())? {
        ...
        let request_sha256 = sql(row.get_ref(3))?
            .as_blob()
            .map_err(|_| Error::Integrity)?;
        let payload_sha256 = sql(row.get_ref(4))?
            .as_blob()
            .map_err(|_| Error::Integrity)?;
        let payload_bytes: i64 = sql(row.get(5))?;
        let digest = sql(row.get_ref(6))?
            .as_blob()
            .map_err(|_| Error::Integrity)?;
        let container = sql(row.get_ref(7))?
            .as_blob()
            .map_err(|_| Error::Integrity)?;
        let material = Material::decode(container)?;
        if material.digest().as_slice() != digest
            || material.operation_id() != operation
            || material.subject() != subject
            || material.kind().number() as i64 != kind
            || material.request_sha256().as_slice() != request_sha256
            || material.payload_sha256().as_slice() != payload_sha256
            || material.payload().len() as i64 != payload_bytes
        {
            return Err(Error::Integrity);
        }
        // The stored original must match the request binding of its command.
        let command = revision_one(c, &operation)?.ok_or(Error::Integrity)?;
        if command.command().request_sha256 != material.request_sha256() {
            return Err(Error::Integrity);
        }
        // The admitted bytes must still satisfy the command's own binding.
        match kind {
            1 => {
                if material.payload().len() as u64 != command.command().request_bytes
                    || material.payload_sha256() != command.command().request_sha256
                {
                    return Err(Error::Integrity);
                }
            }
            2 => {
                if material.payload().len() as u64 > command.command().response_limit {
                    return Err(Error::Integrity);
                }
            }
    ...
    let mut statement = sql(c.prepare(
        "SELECT operation_id,subject,kind,reserved_bytes FROM io_material_reservations",
    ))?;
    let mut rows = sql(statement.query([]))?;
    while let Some(row) = sql(rows.next())? {
        let operation: String = sql(row.get(0))?;
...
impl Store {
    ...
    pub fn reserve_io_materials(
        ...
        // Accepts the in-flight v16→v17 migration window like every other writer.
        ...
        let request_admitted: bool = sql(tx.query_row(
            "SELECT EXISTS(SELECT 1 FROM io_evidence WHERE operation_id=?1 AND kind=1)",
            [&command.operation_id],
            |r| r.get(0),
        ))?;
        let response_admitted: bool = sql(tx.query_row(
        ...
        if incoming > 0 {
            super::event_room(&tx, self.budget, incoming)?;
        }
    ...
    pub fn store_io_material(
        ...
        let existing: Option<Vec<u8>> = sql(tx.query_row(
            "SELECT digest FROM io_evidence WHERE operation_id=?1 AND kind=?2",
            params![operation, kind.number() as i64],
            |r| r.get(0),
        ).optional())?;
        if let Some(existing) = existing {
            if existing.as_slice() != material.digest() {
                return Err(Error::OperationConflict);
            }
    ...
    pub fn io_material(
        ...
        let row: Option<(Vec<u8>, Vec<u8>)> = sql(snapshot.query_row(
            "SELECT digest,container FROM io_evidence WHERE operation_id=?1 AND kind=?2",
            params![operation, kind.number() as i64],
            |r| Ok((r.get(0)?, r.get(1)?)),
        ).optional())?;
        let Some((digest, container)) = row else {
            return Err(Error::EvidenceUnavailable);
        };
        let material = io_evidence::Material::decode(&container)?;
        if material.digest().as_slice() != digest.as_slice()
            || material.operation_id() != operation
            || material.subject() != subject
            || material.kind() != kind
        {
        ...
        let command = revision_one(&snapshot, operation)?.ok_or(Error::Integrity)?;
        if command.command().request_sha256 != material.request_sha256() {
            return Err(Error::Integrity);
        }
        Ok(Some(material))
...
mod disk_tests {
    ...
    fn full_disk_rolls_material_storage_back_without_partial_writes() {
        ...
        let material =
            Material::encode(Kind::Request, "material", "plugin.io-test", payload_sha256, &payload)
                .unwrap();
        let pages: i64 = store
            .connection
