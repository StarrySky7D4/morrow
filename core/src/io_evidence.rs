...
impl Material {
    ...
    pub fn decode(container: &[u8]) -> Result<Self> {
        ...
        preflight(&raw)?;
        let value =
            proto::Material::decode(raw.as_slice()).map_err(|_| Error::Invalid("IO evidence protobuf"))?;
...
mod tests {
    use super::*;
    fn sample() -> Material {
        Material::encode(Kind::Request, "io-operation-1", "plugin.io-test", [2; 32], b"material")
            .unwrap()
    }
    ...
    fn roundtrip_preserves_kind_bindings_and_exact_bytes() {
        for kind in [Kind::Request, Kind::Response] {
            let material =
                Material::encode(kind, "io-operation-1", "plugin.io-test", [2; 32], b"material")
                    .unwrap();
    ...
    fn tampering_truncation_and_trailing_bytes_never_decode() {
        ...
        for position in [0, 8, 18, 40, original.len() - 1] {
            ...
            bytes[position] ^= 0x40;
            assert!(Material::decode(&bytes).is_err(), "tampered byte {position}");
        }
    ...
    fn oversize_materials_and_fields_reject_with_limit() {
        ...
        assert!(matches!(
            Material::encode(Kind::Request, "io-operation-1", "plugin.io-test", [2; 32], &oversized),
            Err(Error::Limit)
        ));
        assert!(matches!(
    ...
    fn identities_digests_lengths_and_containers_are_validated() {
        ...
        assert!(
            Material::encode(Kind::Request, "io-operation-1", "plugin.io-test", [0; 32], b"x")
                .is_err()
        );
        let long_identity = "x".repeat(257);
        assert!(
            Material::encode(Kind::Request, &long_identity, "plugin.io-test", [2; 32], b"x")
                .is_err()
        );
