...
fn legacy(path: &std::path::Path) {
    ...
    c.execute_batch(
        "DROP TABLE operation_evidence; DROP TABLE task_evidence; DROP TABLE audit_identity; DROP TABLE operation_events; DROP TABLE sealed_segments; PRAGMA user_version=4;",
    )
    .unwrap();
}
#[test]
