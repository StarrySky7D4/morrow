...
fn legacy_seven(path: &Path, originals: &[Evidence]) {
    ...
    sql.execute_batch("DROP TABLE io_reservations; DROP TABLE io_intents; DROP TABLE read_archive_costs; DROP TABLE read_archive_totals; DROP TABLE read_captures; DROP TABLE operation_read_archives; DROP TABLE read_archive_parts; DROP TABLE read_archives; DROP TABLE task_evidence_chunks; DROP TABLE evidence_chunks; PRAGMA user_version=7; COMMIT;").unwrap();
}
#[test]
fn same_package_observations_share_real_blocks_and_rebuild_exact_original_containers() {
    let (_dir, path, a, b) = fixture();
    let sql = rusqlite::Connection::open(&path).unwrap();
    assert_eq!(version(&sql), 16);
...
fn schema_eight(path: &Path) {
    let sql = rusqlite::Connection::open(path).unwrap();
    assert_eq!(version(&sql), 16);
    sql.execute_batch("DROP TABLE io_reservations; DROP TABLE io_intents; DROP TABLE read_archive_costs; DROP TABLE read_archive_totals; DROP TABLE read_captures; DROP TABLE operation_read_archives; DROP TABLE read_archive_parts; DROP TABLE read_archives; PRAGMA user_version=8;").unwrap();
}
...
fn format_eight_readonly_then_migration_preserve_unknown_v1_bytes_and_signed_history() {
    ...
    drop(migrated);
    assert_eq!(version(&rusqlite::Connection::open(&path).unwrap()), 16);
...
fn intent_evidence(size: usize) -> Evidence {
    let single = evidence("intent-page", 128 * 1024);
    let o = single.data();
    task_evidence::encode(TaskEvidence {
        schema_version: 2,
        package_archive: o.package_archive.clone(),
        batch: Some(task_evidence::proto::Batch {
            intent_type: "test.projection-facts.v1".into(),
            intent: (0..size)
                .map(|i| ((i * 31 + i / 997) % 251) as u8)
                .collect(),
            total_fuel: 100000,
            observations: vec![task_evidence::proto::Observation {
                invocation: o.invocation.clone(),
...
fn schema_nine(path: &Path) {
    let sql = rusqlite::Connection::open(path).unwrap();
    assert_eq!(version(&sql), 16);
    sql.execute_batch("DROP TABLE io_reservations; DROP TABLE io_intents; DROP TABLE read_archive_costs; DROP TABLE read_archive_totals; DROP TABLE read_captures; DROP TABLE operation_read_archives; DROP TABLE read_archive_parts; DROP TABLE read_archives; PRAGMA user_version=9;").unwrap();
}
#[test]
fn format_nine_preserves_legacy_maximum_intent_and_signature_across_migration() {
    let e = intent_evidence(task_evidence::MAX_LEGACY_BATCH_INTENT_BYTES);
    let (_dir, path, trust, segment) = signed_fixture(std::slice::from_ref(&e));
    schema_nine(&path);
    let before = operation_payload(&path);
    let chunks = stored_chunks(&path);
    let old = Store::open_read_only_audited(&path, trust.clone()).unwrap();
    let loaded = old.operation_evidence("card", "create").unwrap().remove(0);
    assert_eq!(loaded.raw(), e.raw());
    assert_eq!(loaded.container(), e.container());
    drop(old);
    assert_eq!(version(&rusqlite::Connection::open(&path).unwrap()), 9);
    let migrated = Store::open_audited(&path, Default::default(), false, trust).unwrap();
    assert_eq!(
        migrated.operation_evidence("card", "create").unwrap()[0].container(),
        e.container()
    );
    assert_eq!(migrated.sealed_segment(1).unwrap().unwrap(), segment);
    migrated.integrity_check().unwrap();
...
#[test]
fn expanded_intent_roundtrips_in_ten_but_cannot_be_smuggled_into_nine() {
    let e = intent_evidence(8 * 1024 * 1024 + 4096);
    let (_dir, path, trust, segment) = signed_fixture(std::slice::from_ref(&e));
    let valid = Store::open_read_only_audited(&path, trust.clone()).unwrap();
    let loaded = valid
        .operation_evidence("card", "create")
...
#[cfg(feature = "fault-injection")]
#[test]
fn projection_capacity_migration_crashes_leave_whole_nine_or_ten() {
    for point in [
        "projection-evidence-migration-before-commit",
        "projection-evidence-migration-after-commit",
    ] {
        let (_dir, path, a, b) = fixture();
        schema_nine(&path);
        let before = operation_payload(&path);
        ...
        assert_eq!(stored_chunks(&path), chunks);
        let reopened = Store::open_existing(&path, Default::default()).unwrap();
        let loaded = reopened.operation_evidence("card", "create").unwrap();
        for (actual, expected) in loaded.iter().zip([&a, &b]) {
            assert_eq!(actual.raw(), expected.raw());
            assert_eq!(actual.container(), expected.container());
            assert_eq!(actual.digest(), expected.digest());
        }
