...
fn noncanonical_unknown_and_duplicate_field_encodings_are_rejected() {
    ...
    long.extend([1u8; 33]);
    assert!(matches!(Material::decode(&pack(&long)), Err(Error::Limit)));
    assert!(matches!(
...
fn reconciliation_releases_only_never_admitted_quota() {
    ...
    store
        .release_io_materials(SUBJECT, "early-release")
        .unwrap();
    store.integrity_check().unwrap();
}
...
fn downgrade_with_new_objects_is_refused_and_honest_v16_migrates_cleanly() {
    ...
    rusqlite::Connection::open(&path)
        .unwrap()
        .execute_batch(
            "DROP TABLE io_evidence; DROP TABLE IF EXISTS outbound_authorities; DROP TABLE IF EXISTS service_authority_identity; DROP TABLE IF EXISTS service_authorities; DROP TABLE IF EXISTS service_configs; DROP INDEX IF EXISTS io_evidence_kind; DROP TABLE io_material_reservations; PRAGMA user_version=16;",
        )
        .unwrap();
...
#[test]
fn cancelled_history_rejects_new_reservation() {
    let (_dir, path, mut store) = setup();
    let record = prepared("cancelled-review");
    ...
    sql.execute("INSERT INTO io_material_reservations(operation_id,kind,subject,reserved_bytes) VALUES(?1,2,?2,?3)",rusqlite::params![record.command().operation_id,SUBJECT,io_evidence::max_container_bytes(record.command().response_limit).unwrap() as i64]).unwrap();
    assert!(
        store.integrity_check().is_err(),
        "Cancelled revision 2 with leaked reservation passed integrity"
    );
}
#[test]
fn read_rejects_payload_not_bound_to_original_command() {
    let (_dir, path, mut store) = setup();
    let record = prepared("changed-review");
    append(&mut store, &record);
    store
        .reserve_io_materials(record.command(), || Ok(()))
        .unwrap();
    let good = material(Kind::Request, "changed-review", &request_payload());
    store
        .store_io_material(SUBJECT, Kind::Request, &good, || Ok(()))
        .unwrap();
    let changed = material(Kind::Request, "changed-review", b"unrelated request bytes");
    let sql = rusqlite::Connection::open(&path).unwrap();
    sql.execute("UPDATE io_evidence SET digest=?1,container=?2,payload_sha256=?3,payload_bytes=?4 WHERE operation_id=?5",rusqlite::params![changed.digest().as_slice(),changed.container(),changed.payload_sha256().as_slice(),changed.payload().len() as i64,"changed-review"]).unwrap();
    assert!(store.integrity_check().is_err());
    assert!(
        store
            .io_material(SUBJECT, "changed-review", Kind::Request)
            .is_err(),
        "read delivered invalid command-bound material even though integrity_check rejects it"
    );
}
#[test]
fn decode_preserves_accepted_original_container() {
    let original = material(Kind::Request, "container-review", &request_payload());
    ...
    container.extend_from_slice(io_evidence::MAGIC);
    container.extend_from_slice(&1u16.to_le_bytes());
    container.extend_from_slice(&(raw.len() as u32).to_le_bytes());
    container.extend_from_slice(&(block.len() as u32).to_le_bytes());
    container.extend_from_slice(&Sha256::digest(raw));
    container.extend_from_slice(&block);
    let decoded = Material::decode(&container).unwrap();
    assert_eq!(decoded.raw(), raw);
    assert_eq!(
        decoded.container().len(),
        container.len(),
        "valid literal-only LZ4 original was silently recompressed"
    );
    assert_eq!(decoded.container(), container);
    let (_dir, _path, mut store) = setup();
    let record = prepared("container-review");
    append(&mut store, &record);
    store
        .reserve_io_materials(record.command(), || Ok(()))
        .unwrap();
    store
        .store_io_material(SUBJECT, Kind::Request, &decoded, || Ok(()))
        .unwrap();
    // Same canonical PB may have another valid compression. A retry validates
    // its semantic original but must never replace the first stored container.
    let mut authorized = false;
    store
        .store_io_material(SUBJECT, Kind::Request, &original, || {
            authorized = true;
            Ok(())
        })
        .unwrap();
    assert!(authorized);
    let restored = store
        .io_material(SUBJECT, "container-review", Kind::Request)
        .unwrap()
        .unwrap();
...
#[test]
fn exact_admission_rejects_corrupted_existing_material() {
    let (_dir, path, mut store) = setup();
    let record = prepared("retry-review");
    append(&mut store, &record);
    store
        .reserve_io_materials(record.command(), || Ok(()))
        .unwrap();
    let good = material(Kind::Request, "retry-review", &request_payload());
    store
        .store_io_material(SUBJECT, Kind::Request, &good, || Ok(()))
        .unwrap();
    let sql = rusqlite::Connection::open(&path).unwrap();
    sql.execute(
        "UPDATE io_evidence SET container=X'00' WHERE operation_id='retry-review'",
        [],
    )
    .unwrap();
    assert!(
        store
            .io_material(SUBJECT, "retry-review", Kind::Request)
            .is_err()
    );
    assert!(
        store
            .store_io_material(SUBJECT, Kind::Request, &good, || Ok(()))
            .is_err(),
        "exact re-admission returned success despite unreadable stored original"
    );
}
#[test]
fn snapshot_preserves_original_and_reservation_without_source_database() {
    let (dir, path, mut store) = setup();
    let record = prepared("snapshot-review");
    append(&mut store, &record);
    store
        .reserve_io_materials(record.command(), || Ok(()))
        .unwrap();
    let good = material(Kind::Request, "snapshot-review", &request_payload());
    store
        .store_io_material(SUBJECT, Kind::Request, &good, || Ok(()))
        .unwrap();
    let usage = store.io_material_reservation_usage().unwrap();
    let snapshot = dir.path().join("independent-snapshot.db");
    store.snapshot_to(&snapshot, 64 * 1024 * 1024).unwrap();
    drop(store);
    std::fs::remove_file(path).unwrap();
    let reopened = Store::open_existing(&snapshot, EventBudget::default()).unwrap();
    reopened.integrity_check().unwrap();
    let restored = reopened
        .io_material(SUBJECT, "snapshot-review", Kind::Request)
        .unwrap()
        .unwrap();
    assert_eq!(restored.container(), good.container());
    assert_eq!(restored.raw(), good.raw());
    assert_eq!(restored.digest(), good.digest());
    assert_eq!(reopened.io_material_reservation_usage().unwrap(), usage);
}

#[test]
fn oversized_sql_material_and_revision_one_are_rejected_before_decode() {
    for oversized_history in [false, true] {
        let (_dir, path, mut store) = setup();
        let record = prepared("sql-limit");
        append(&mut store, &record);
        store
            .reserve_io_materials(record.command(), || Ok(()))
            .unwrap();
        let good = material(Kind::Request, "sql-limit", &request_payload());
        store
            .store_io_material(SUBJECT, Kind::Request, &good, || Ok(()))
            .unwrap();
        let sql = rusqlite::Connection::open(&path).unwrap();
        if oversized_history {
            sql.execute(
                "UPDATE operations SET payload=zeroblob(?1) WHERE id=?2",
                rusqlite::params![
                    (morrow_core::io_intent::MAX_CONTAINER_BYTES + 1) as i64,
                    record.event_id()
                ],
            )
            .unwrap();
            ...
            sql.execute(
                "UPDATE io_evidence SET container=zeroblob(?1)",
                [(io_evidence::MAX_CONTAINER_BYTES + 1) as i64],
            )
            .unwrap();
        }
        assert!(matches!(
            store.io_material(SUBJECT, "sql-limit", Kind::Request),
            Err(Error::Limit | Error::Integrity)
        ));
        assert!(
            store
                .store_io_material(SUBJECT, Kind::Request, &good, || Ok(()))
...
#[test]
fn read_and_retry_reject_corrupted_mirror_columns() {
    for statement in [
        "UPDATE io_evidence SET payload_bytes=payload_bytes+1",
        "UPDATE io_evidence SET payload_sha256=zeroblob(32)",
        "UPDATE io_evidence SET request_sha256=zeroblob(32)",
        "UPDATE io_evidence SET subject='another-subject'",
    ] {
        let (_dir, path, mut store) = setup();
        let record = prepared("mirror");
        append(&mut store, &record);
        store
            .reserve_io_materials(record.command(), || Ok(()))
            .unwrap();
        let good = material(Kind::Request, "mirror", &request_payload());
        store
            .store_io_material(SUBJECT, Kind::Request, &good, || Ok(()))
            .unwrap();
        rusqlite::Connection::open(&path)
            ...
            .unwrap();
        assert!(
            store.io_material(SUBJECT, "mirror", Kind::Request).is_err(),
            "{statement}"
        );
        assert!(
            store
...
#[test]
fn material_reservation_needs_bytes_not_an_unused_event_slot() {
    let (_dir, path, mut store) = setup();
    let record = prepared("last-event-slot");
    append(&mut store, &record);
    drop(store);
    let mut store = Store::open_existing(
        &path,
    ...
    store
        .reserve_io_materials(record.command(), || Ok(()))
        .unwrap();
    assert_eq!(store.io_material_reservation_usage().unwrap().0, 2);
    assert!(matches!(
        store.reserve_io_intent_followup(record.command(), || Ok(())),
        Err(Error::EventCapacity)
    ));
...
#[test]
fn response_read_rejects_over_limit_or_pre_dispatch_material() {
    for before_dispatch in [false, true] {
        let (_dir, path, mut store) = setup();
        let record = prepared("response-bound");
        append(&mut store, &record);
        store
            .reserve_io_materials(record.command(), || Ok(()))
            .unwrap();
        if !before_dispatch {
            store
                .reserve_io_intent_followup(record.command(), || Ok(()))
                .unwrap();
            append(&mut store, &record.propose_dispatch_boundary().unwrap());
        }
        let bad = material(
            Kind::Response,
            "response-bound",
            &vec![1; if before_dispatch { 1 } else { 4097 }],
        );
        let sql = rusqlite::Connection::open(&path).unwrap();
        sql.execute("DELETE FROM io_material_reservations WHERE kind=2", [])
            .unwrap();
        sql.execute("INSERT INTO io_evidence(digest,operation_id,subject,kind,request_sha256,payload_sha256,payload_bytes,container) VALUES(?1,?2,?3,2,?4,?5,?6,?7)",rusqlite::params![bad.digest().as_slice(),bad.operation_id(),SUBJECT,bad.request_sha256().as_slice(),bad.payload_sha256().as_slice(),bad.payload().len() as i64,bad.container()]).unwrap();
        assert!(matches!(
            store.io_material(SUBJECT, "response-bound", Kind::Response),
            Err(Error::Integrity)
        ));
        assert!(store.integrity_check().is_err());
    }
