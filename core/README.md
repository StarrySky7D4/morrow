...
这是独立于 Flutter 的 Rust 契约实现，当前版本 `0.1.9-test.10`。阶段为 M0／M1／M2 的部分交付，**尚未成为工作台的数据后端**。旧 Flutter 保存路径仍是旧应用唯一权威；新 SQLite 数据库仅由明确指定路径的实验 CLI／可信宿主访问，没有自动迁移或写入旧资料的入口。构建不运行应用、不读取用户资料。
...
- 实验库格式 4，拒绝格式 3；运行期协议 6 更新了内容契约摘要，但三类记录当前仅走可信本地 API／CLI 和 Web 实验管理入口，尚不是通用插件运行期命令。Web 使用 morrow-test10 命名空间。
