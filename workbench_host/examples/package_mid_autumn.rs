...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    .into_iter()
    .map(
        |(name, input, output, max_input, max_output)| TransformHandler {
            handler: name.into(),
    ...
    .collect();
    let mut manifest =
        Package::manifest_for_transform("org.morrow.theme.mid-autumn", "1.0.0", &module, handlers);
    ...
    if std::path::Path::new(&output).exists() {
        ...
        if old.manifest().package_version == package.manifest().package_version
            && old.digest() != package.digest()
        {
            return Err(
                "Theme bytes changed: bump the theme package version before replacing this file"
                    .into(),
            );
        }
    }
    ...
    println!(
        "Independent Mid-Autumn theme package built ({} bytes)",
        package.archive().len()
    );
    Ok(())
}
