...
use super::*;
use morrow_core::{task_evidence::Evidence, transaction::{self, Lookup, proto::command::Action as StoredAction}};
...
impl Workbench {
    ...
    pub fn operation_evidence(&self, card: &str, operation: &str) -> Result<Vec<Evidence>> {
        Ok(self.host.store_local().operation_evidence(card, operation)?)
    }
    pub(super) fn retry_observed(
        &mut self, operation: &str, id: &str, intent: &Request, revision: Option<u64>,
    ) -> Result<Option<Record>> {
        if matches!(self.host.store_local().lookup(operation)?, Lookup::Absent) {
            return Ok(None);
        }
        let (commit, expected_receipt) = self.host.store_local().operation_commit(id, operation)?
        ...
        let transform = invocation.transform().ok_or("original task was not a transform")?;
        ...
        let (record, receipt) = match command.action.ok_or("missing original command")? {
            StoredAction::CreateCard(raw) if revision.is_none() && intent.action == Action::Create => {
                ...
                let result = self.host.create_content_with_evidence(
                    self.pool.root(self.plugin.as_ref().ok_or("plugin unavailable")?)?.connection(),
                    operation, &card, &evidence, || now(start),
                );
            ...
            StoredAction::SetContent(value) if revision == Some(value.expected_revision) => {
                ...
                let attachments = value.attachments.map(|list| list.items.into_iter().map(|a| {
                    Ok(Attachment {
                        id: a.id, display_name: a.display_name, media_type: a.media_type,
                        byte_length: a.byte_length,
                        sha256: a.sha256.try_into().map_err(|_| "invalid historical attachment digest")?,
                    })
                }).collect::<Result<Vec<_>>>()).transpose()?;
                let change = ContentChange {
                    operation_id: operation.into(), card_id: value.card_id,
                    expected_revision: value.expected_revision, title: value.title,
                    body: value.body, preview_text: value.preview_text, attachments,
                };
                self.grant(id, GrantKind::EditContent)?;
                let start = self.start;
                let result = self.host.edit_content_with_evidence(
                    self.pool.root(self.plugin.as_ref().ok_or("plugin unavailable")?)?.connection(),
                    &change, &evidence, || now(start),
                );
