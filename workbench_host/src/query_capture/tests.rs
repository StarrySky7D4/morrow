...
fn replay_production(
    ...
    assert_eq!(manifest.metadata_type(), Some("morrow.query.end.v1"));
    assert_eq!(
    ...
    assert_eq!(first.type_id(), "morrow.query.package.v1");
...
fn same_operation_retries_original_results_without_guest_runs_and_survives_reopen() {
    ...
    let error = host
        .query_with_operation("fixed", "概览", "全部", "other intent", "标题排序")
        .unwrap_err();
...
fn production_multiphase_archive_replays_all_originals_after_backup_and_source_removal() {
    ...
    let mut baseline = host.host.store_local().open_card_snapshot().unwrap();
...
fn ready_after_seal_failure_retries_original_result_without_another_guest_call() {
    ...
    host.host.fail_next_seal_for_test();
    ...
    let original = host
        ...
        .unwrap()
        .unwrap();
    assert_eq!(host.host.store_local().pending_usage().unwrap().0, 1);
...
fn existing_content_operation_cannot_become_a_query_retry_loop() {
    ...
    assert_eq!(
        host.query_with_operation("fresh-query", "概览", "全部", "", "最近添加")
            .unwrap(),
        ["a"]
    );
    host.finish().unwrap();
