...
当前开发版本为 `0.1.9-test.4+13`。`test.1` 是 `0.1.x` 最后一个兼容现有数据类型的测试版；从 `test.2` 起以 `0.1.9-test.x` 逐步推进大规模重写，后续可能包含破坏性数据变更。核心架构与数据模型锚定、插件系统及声明支持的平台完成验收后发布 `0.2.0`。最小插件原型跑通不等于系统完成；阶段任务、验收范围与版本规则见 [未来路线](docs/FUTURE_ROADMAP.md)。

`test.4` 在独立 [Rust 内容核心](core/README.md) 中建立原生 SQLite 事务：卡片、操作结果与待封存事件原子写入，支持操作去重、提交前权限核对和独立 CLI。提交各边界的子进程退出恢复已有测试。Windows FFI 和 Chrome Worker 保持协议往返验证；普通 JavaScript 接入及浏览器持久化仍待完成。工作台尚未切换至新核心，不会自动迁移旧资料。阶段证据与下一步见 [test.4 重构记录](reports/0.1.9-test.4-refactor.md)。
