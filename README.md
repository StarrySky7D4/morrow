...
当前开发版本为 `0.1.9-test.5+14`。`test.1` 是 `0.1.x` 最后一个兼容现有数据类型的测试版；从 `test.2` 起以 `0.1.9-test.x` 逐步推进大规模重写，后续可能包含破坏性数据变更。核心架构与数据模型锚定、插件系统及声明支持的平台完成验收后发布 `0.2.0`。最小插件原型跑通不等于系统完成；阶段任务、验收范围与版本规则见 [未来路线](docs/FUTURE_ROADMAP.md)。

`test.5` 在独立 [Rust 内容核心](core/README.md) 中加入附件分块暂存、原字节校验／导出、卡片与历史事件的原子引用，以及显式退休后的受保护回收。原生 SQLite 事务与操作去重继续保留，附件相关进程中断恢复已有测试。新数据库格式为 2，尚未切换工作台或迁移旧资料；浏览器持久化和插件运行仍待完成。[本轮重构记录](reports/0.1.9-test.5-refactor.md)。
