...
用 Flutter 构建的灵感工作台。当前开发版为 **0.1.9-test.23**（尚未发布）：Windows 默认启动 Rust 工作台插件与独立核心数据库，Flutter 负责界面和平台媒体承载；Web／Android 暂保留原路径。旧 test.1 数据不会自动导入或覆盖。当前迁移结果与容量边界见 [test.1 功能对照](docs/TEST1_RUST_PARITY.md)。
...
当前开发版本为 `0.1.9-test.23+28`；已发布的 test.14 为 Windows x64 测试预览构建，不是稳定版。`test.1` 是 `0.1.x` 最后一个兼容现有数据类型的测试版；从 `test.2` 起以 `0.1.9-test.x` 逐步推进大规模重写，后续可能包含破坏性数据变更。核心架构与数据模型锚定、插件系统及声明支持的平台完成验收后发布 `0.2.0`。最小插件原型跑通不等于系统完成；阶段任务、验收范围与版本规则见 [未来路线](docs/FUTURE_ROADMAP.md)。
