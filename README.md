...
当前开发版本为 `0.1.9-test.8+17`。`test.1` 是 `0.1.x` 最后一个兼容现有数据类型的测试版；从 `test.2` 起以 `0.1.9-test.x` 逐步推进大规模重写，后续可能包含破坏性数据变更。核心架构与数据模型锚定、插件系统及声明支持的平台完成验收后发布 `0.2.0`。最小插件原型跑通不等于系统完成；阶段任务、验收范围与版本规则见 [未来路线](docs/FUTURE_ROADMAP.md)。

`test.8` 将原生 Dart 接入 [Rust 宿主分派](core/README.md)，可在独立实验数据库中提交修改、读取摘要和查询操作结果。结果查询单独授权并限制卡片范围；已验证丢失响应后查回结果、完整 UInt64、缓冲区耗尽时不写入、句柄失效及重启恢复。Web 同步使用二进制协议 v4。工作台和旧资料未切换，插件执行与审计重放继续推进。[本轮重构记录](reports/0.1.9-test.8-refactor.md)。
