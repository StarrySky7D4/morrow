...
当前开发版本为 `0.1.9-test.9+18`。`test.1` 是 `0.1.x` 最后一个兼容现有数据类型的测试版；从 `test.2` 起以 `0.1.9-test.x` 逐步推进大规模重写，后续可能包含破坏性数据变更。核心架构与数据模型锚定、插件系统及声明支持的平台完成验收后发布 `0.2.0`。最小插件原型跑通不等于系统完成；阶段任务、验收范围与版本规则见 [未来路线](docs/FUTURE_ROADMAP.md)。

`test.9` 增加按卡片与附件分别授权的分块读取：每包绑定修订，接收端按顺序校验并核对完整文件 SHA-256。原生 Dart 与 Web 共用 Rust 分派及完整文件校验器，实验数据库升至格式 3，旧实验数据库没有自动迁移。工作台和旧资料未切换，插件执行与审计重放继续推进。[本轮重构记录](reports/0.1.9-test.9-refactor.md)。
