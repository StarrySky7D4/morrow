...
当前开发版本为 `0.1.9-test.6+15`。`test.1` 是 `0.1.x` 最后一个兼容现有数据类型的测试版；从 `test.2` 起以 `0.1.9-test.x` 逐步推进大规模重写，后续可能包含破坏性数据变更。核心架构与数据模型锚定、插件系统及声明支持的平台完成验收后发布 `0.2.0`。最小插件原型跑通不等于系统完成；阶段任务、验收范围与版本规则见 [未来路线](docs/FUTURE_ROADMAP.md)。

`test.6` 新增独立 [Web 核心](core-web/README.md)：普通 Dart JavaScript 通过 Rust/Wasm 处理完整 UInt64 协议，浏览器 OPFS 复用既有 Rust 内容与附件事务。已验证 Worker 中断恢复、重复提交、原件暂存与跨页面恢复。当前仍为实验入口，工作台和旧资料未切换；插件执行与审计重放继续推进。[本轮重构记录](reports/0.1.9-test.6-refactor.md)。
