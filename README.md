...
Windows 原生承载层始终透明，由 Flutter 在圆角内绘制默认、纯色和纹理背景，避免视频圆角外残留原生实色方角。Windows 透明模式会透出窗口后面的桌面或其他窗口；Web 输出透明画布，透出宿主网页背景，独立浏览器页面通常显示浏览器的白色底，不会透出操作系统桌面。透明画布不会铺设假的棋盘格背景。玻璃面板自身保留渐变遮罩和描边；Flutter 背景模糊作用于应用内绘制内容，不会对后面的其他 Windows 窗口做局部物理折射或模糊。
