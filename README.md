...
当前开发版本为 `0.1.9-test.3+12`。`test.1` 是 `0.1.x` 最后一个兼容现有数据类型的测试版；从 `test.2` 起以 `0.1.9-test.x` 逐步推进大规模重写，后续可能包含破坏性数据变更。核心架构与数据模型锚定、插件系统及声明支持的平台完成验收后发布 `0.2.0`。最小插件原型跑通不等于系统完成；阶段任务、验收范围与版本规则见 [未来路线](docs/FUTURE_ROADMAP.md)。

`test.3` 在独立 [Rust 内容核心](core/README.md) 上加入协议摘要校验、宿主权限／生命周期状态原型，以及 [Dart 原生与 Web Wasm 客户端](packages/morrow_core_client/README.md)。Windows FFI 和 Chrome Worker 消息往返已实测；普通 JavaScript 编译仍有精确整数限制。工作台尚未切换至新核心，当前不会自动迁移旧资料。阶段证据与下一步见 [test.3 重构记录](reports/0.1.9-test.3-refactor.md)。
