...
mod native {
    ...
    fn migration_preserves_old_event_bytes_and_rejects_false_downgrade() {
        ...
        let sql = rusqlite::Connection::open(&path).unwrap();
        sql.execute_batch("DROP TABLE IF EXISTS tls_identities; DROP TABLE IF EXISTS outbound_authorities; DROP TABLE IF EXISTS service_authority_identity; DROP TABLE IF EXISTS service_authorities; DROP TABLE service_configs;").unwrap();
        drop(sql);
        let store = Store::open_existing(&path, Default::default()).unwrap();
    ...
    fn corrupted_index_identity_payload_and_schema_fail_closed() {
        for case in 0..4 {
            ...
            sql.execute_batch(match case {
                ...
                _ => "DROP TABLE IF EXISTS tls_identities; DROP TABLE IF EXISTS outbound_authorities; DROP TABLE IF EXISTS service_authority_identity; DROP TABLE IF EXISTS service_authorities; DROP TABLE service_configs;",
            })
            .unwrap();
            drop(sql);
            assert!(
...
mod crash {
    ...
    fn migration_crash_preserves_whole_old_or_new_schema() {
        for (point, version) in [
            ...
            let sql = rusqlite::Connection::open(&path).unwrap();
            sql.execute_batch("DROP TABLE IF EXISTS tls_identities; DROP TABLE IF EXISTS outbound_authorities; DROP TABLE IF EXISTS service_authority_identity; DROP TABLE IF EXISTS service_authorities; DROP TABLE service_configs; PRAGMA user_version=17;")
                .unwrap();
            drop(sql);
