...
use crate::{Error, Result, dependency_call_capnp as wire, identity};
use capnp::{
...
pub fn schema_digest() -> [u8; 32] {
    crate::runtime::schema_digest(include_bytes!("../schemas/dependency_call.capnp"))
}
fn invalid<T>(_: T) -> Error {
    Error::Invalid("dependency call")
}
