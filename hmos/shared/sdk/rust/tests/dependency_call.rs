...
fn schema_snapshots_share_lf_normalized_digest() {
    let source = String::from_utf8(include_bytes!("../contracts/dependency_call.capnp").to_vec())
        .unwrap()
        .replace("\r\n", "\n");
    let sdk =
        String::from_utf8(include_bytes!("../../../core/schemas/dependency_call.capnp").to_vec())
            .unwrap()
            .replace("\r\n", "\n");
