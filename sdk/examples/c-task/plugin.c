...
int32_t morrow_run(void) {
 ...
 mp_task* task=0;mp_span command;uint32_t size=0,length=0;int32_t result=-1;
 ...
 if(mp_task_get_command(task,&command,sizeof(command))!=MP_CODEC_OK)goto done;
 mp_host_v1 host=mp_wasm_host();
 if(mp_exchange(&host,command.data,command.length,response,MP_MAX_MESSAGE_BYTES,&size)!=MP_OK)goto done;
