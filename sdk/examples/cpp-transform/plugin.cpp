...
extern "C" int32_t morrow_run(){
 ...
 if(text(t.handler)=="bytes.reverse")std::reverse(value.begin(),value.end());else if(text(t.handler)=="bytes.ascii-uppercase"){for(auto& b:value)if(b>='a'&&b<='z')b=static_cast<uint8_t>(b-32);}else if(text(t.handler)=="bytes.require-ascii"){
   if(std::any_of(value.begin(),value.end(),[](uint8_t b){return b>127;})){
     auto failed=task.fail(MP_TASK_UNSUPPORTED_INPUT,"Input contains non-ASCII bytes");
     if(failed.status!=MP_CODEC_OK)return -1;
     return mp_wasm_task_complete(failed.bytes.data(),static_cast<uint32_t>(failed.bytes.size()));
   }
 }else return -1;
