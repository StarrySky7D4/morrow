...
try {
  ...
  & cargo clippy --locked --manifest-path core-web/Cargo.toml --target-dir build/core-test.7 --target wasm32-unknown-unknown --all-features -- -D warnings
  ...
  foreach ($profile in @('main', 'fault')) {
    ...
    & cargo build --locked --manifest-path core-web/Cargo.toml --target-dir build/core-test.7 --target wasm32-unknown-unknown --release @features
    if ($LASTEXITCODE -ne 0) { throw "Web core build failed: $profile" }
    & $WasmBindgen build/core-test.7/wasm32-unknown-unknown/release/morrow_web_core.wasm --target web --out-dir "build/core-test.7/web-store/$profile"
    if ($LASTEXITCODE -ne 0) { throw "Web binding generation failed: $profile" }
  }
  Copy-Item core-web/web/* build/core-test.7/web-store/ -Force
  Push-Location packages/morrow_core_client
  try {
    & $Dart compile js -O2 bin/js_probe.dart -o ../../build/core-test.7/web-store/js_probe.js
    if ($LASTEXITCODE -ne 0) { throw 'Ordinary Dart JavaScript compilation failed' }
  } finally { Pop-Location }
  & cargo run --locked --manifest-path core/Cargo.toml --target-dir build/core-test.7 --example protocol_vectors -- build/core-test.7/web-store/vectors
  if ($LASTEXITCODE -ne 0) { throw 'Protocol vectors failed' }
  & cargo run --locked --manifest-path core/Cargo.toml --target-dir build/core-test.7 --example web_storage_vectors -- generate build/core-test.7/web-store/vectors/high.morrow
  ...
  & cargo run --locked --manifest-path core/Cargo.toml --target-dir build/core-test.7 --example web_storage_vectors -- verify build/core-test.7/browser-card.morrow
