...
try {
  ...
  & cargo clippy --locked --manifest-path core/Cargo.toml --target-dir build/core-test.10 --all-targets -- -D warnings
  ...
  & cargo run --locked --manifest-path core/Cargo.toml --target-dir build/core-test.10 --bin morrow-core-check -- self-check
  if ($LASTEXITCODE -ne 0) { throw 'Core self-check failed' }
  if ($Web) {
    & cargo check --locked --manifest-path core/Cargo.toml --target-dir build/core-test.10 --target wasm32-unknown-unknown --lib
