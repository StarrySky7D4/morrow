...
$webOutput = Join-Path $repo "dist/daemon-$version-web.zip"
if ($IncludeWeb) {
...
$artifacts = @($output)
if ($IncludeWeb) {
  $webStage = Join-Path (Split-Path $stage -Parent) 'daemon-web'
...
$checksums = Join-Path $repo "dist/daemon-$version-SHA256SUMS.txt"
