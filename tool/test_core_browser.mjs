...
const webFolder=process.argv.includes('--store')?'build/core-test.7/web-store':'build/core-test.7/web';
...
const server = createServer(async (request, response) => {
  try {
    ...
    if(process.argv.includes('--store')&&request.method==='POST'&&route==='/capture/browser-card.morrow'){
      ...
      await writeFile(path.join(root,'build/core-test.7/browser-card.morrow'),Buffer.concat(chunks));response.writeHead(204).end();return;
    }
